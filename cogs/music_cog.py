...
class music_cog(commands.Cog):
    ...
    async def play_music(self, vc, server_id):                                                     #start to play the song, usually when no song is playing currently
        if (len(self.music_queue[server_id]) > 0):
            ...
            m_url = self.music_queue[server_id][0][0]['source']
            self.message_embed[server_id] = discord.Embed()
            self.message_embed[server_id].description = f"TOCANDO AGORA: [{self.music_queue[server_id][0][0]['title']}]({self.music_queue[server_id][0][0]['yt_url']})"
    ...
    async def join(self, ctx):                                          #join the voice channel of the command sender
        ...
        if ctx.author.voice is None:
            self.message_embed[server_id].description = "Você não está em um canal de voz."
    ...
    async def play(self, ctx, *args):                                                              #play song in the voice channel of the command sender
        ...
        if (ctx.author.voice is None):                                                             #join the command sender's voice channel or ask the sender to join one
            self.message_embed[server_id].description = "Conecte-se a um canal de voz para reproduzir música."
            await ctx.send(embed=self.message_embed[server_id])
            ...
            self.message_embed[server_id].description = "Você tem que estar no mesmo canal de voz do bot"
            await ctx.send(embed=self.message_embed[server_id])            
        else:
        ...
        if (ctx.author.voice is not None and ctx.author.voice.channel == voice_channel):           #add the song to the queue and start playing
            ...
            if (type(song) == type(True)):
                ...
                if (self.server_status[server_id]["is_playing"] == True):
                    self.message_embed[server_id].description = f"Música adicionada à fila: [{song['title']}]({song['yt_url']})"
                    await ctx.send(embed=self.message_embed[server_id])  
                else:
    ...
    async def queue(self, ctx):                                                                                 #show the song queue
        ...
        if retval != "Song queue:\n":
            ...
            self.message_embed[server_id].description = "Nenhuma música na fila"
        await ctx.send(embed=self.message_embed[server_id])
    ...
    async def remove(self, ctx, q_num):                                                                         #remove songs in the song queue
        ...
        if (q_num == 'all'):                                                                                    #remove all the songs
            self.message_embed[server_id].description = "Todas as músicas na fila foram removidas"
            self.music_queue[server_id] = []    
        elif (q_num.isdigit() == False or int(q_num) < 1 or int(q_num) > (len(self.music_queue[server_id]))):   #report for wrong input
            self.message_embed[server_id].description = "Insira um número de fila correto"
        else:
            self.message_embed[server_id].description = f"Música removida: [{self.music_queue[server_id][int(q_num)-1][0]['title']}]({self.music_queue[server_id][int(q_num)-1][0]['yt_url']})"           
    ...
    async def pause(self, ctx):                                     #pause the current song
        ...
        if (self.server_status.get(server_id) == None or self.server_status[server_id]["is_playing"] == False):
            self.message_embed[server_id].description = "Nenhuma música está tocando agora"    
        elif (ctx.voice_client.is_paused() == True):
            self.message_embed[server_id].description = "A música já está pausada"
        else:
            ctx.voice_client.pause()
            self.message_embed[server_id].description = "MÚSICA PAUSADA"
        await ctx.send(embed=self.message_embed[server_id])
    ...
    async def resume(self, ctx):                                    #resume the current song
        ...
        if (self.server_status.get(server_id) == None or self.server_status[server_id]["is_playing"] == False):
            ...
            self.message_embed[server_id].description = "A música já está tocando"
        else:
            ctx.voice_client.resume()
            self.message_embed[server_id].description = "Música retomada"
        await ctx.send(embed=self.message_embed[server_id])
    ...
    async def loop(self, ctx):                                      #loop the current song
        ...
        if (self.server_status[server_id]["is_looping"] == False):
            self.message_embed[server_id].description = "Loop esta ativado"
        else:
            self.message_embed[server_id].description = "Loop is desativado"
